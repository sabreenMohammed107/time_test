...
jQuery(document).ready(function($)
{
	        ...
	        filterButton.on('click', function()
	        {
	        	$('.product-grid').isotope({
		            filter: function()
		            {
			        	...
			        	var priceMin = parseFloat(priceRange.split('-')[0].replace('EGP ', ''));
			        	var priceMax = parseFloat(priceRange.split('-')[1].replace('EGP ', ''));
			        	var itemPrice = $(this).find('.product_price').clone().children().remove().end().text().replace( 'EGP ', '' );
	...
	*/
    ...
    function initPriceSlider()
    {
		...
		$("#amount").val("EGP " + $("#slider-range").slider("values", 0) + " - EGP " + $("#slider-range").slider("values", 1));
    }
